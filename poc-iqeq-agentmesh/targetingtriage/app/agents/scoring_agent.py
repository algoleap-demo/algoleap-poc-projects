...
def run_scoring_agent(raw_data: dict):
    tracker.emit("ag-ml", "started", "Initializing XGBoost inference engine...")
    ...
    try:
        ...
        tracker.emit("ag-ml", "error", error_msg)
        raise
        
    tracker.emit("ag-ml", "processing", "Computing features for accounts...")
    ...
    # 3. Predict
    tracker.emit("ag-ml", "processing", f"Running model scoring on {len(X)} records...")
    ...
    tracker.emit("ag-ml", "completed", f"Scoring complete. Average Propensity Score: {avg_score:.2f}")
    return results
